...
public class Forum {
    ...
    private static void forum(SLL<Discussion> health, SLL<Discussion> finance) {
        SLLNode<Discussion> prva = health.getFirst();
        SLLNode<Discussion> vtora = finance.getFirst();
        SLLNode<Discussion> temp2 = finance.getFirst();
        SLLNode<Discussion> temp = health.getFirst();
        int max = 0;
        int min = 1000;
        while (prva != null) {
            int korisnici = prva.element.getUsers();
            int popularnost = prva.element.getPopularity();
            int vaznost = korisnici*popularnost;
            if (vaznost > max) {
                max = vaznost;
                temp.element = prva.element;
            }
            prva = prva.succ;
        }

        while (vtora != null) {
            int koris = vtora.element.getUsers();
            int popul = vtora.element.getPopularity();
            int vazn = koris*popul;
            if (vazn < min) {
                min = vazn;
                temp2.element = vtora.element;
            }
